...
Related hard requirement (from a live incident, restated because the console is the surface where it bites): the PA arm relay de-energizes if its inputs do not refresh within 10 s. The enabling input is the radio's `/state`, and the radio bridge publishes `/state` only on change. When the radio is idle-but-healthy this starves the heartbeat. The interlock drops out. Any reconstruction of the radio bridge must republish radio state at least every 5 s. Or it must give a liveness mechanism with the same effect (see `03-components/flexbridge.md`, `06-safety.md`). The console cannot fix this. It must merely survive it and render it truthfully. (2026-09-15: the PA ARM panel was removed from the console — a manual operator ARM/SAFE on a sequencer-owned safety interlock was a regression — so the drop-out itself is no longer rendered as a state flip; the console still monitors the `hf/pa-arm` slot and reports it in the faults bar when it goes dark.)
