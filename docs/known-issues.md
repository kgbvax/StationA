...
Update (2026-09-13, sat-ops plan U10/U11): **still open — no longer a descope question, now a flash-and-verify question.** The firmware exists as a component: `m5stamp-pol-ctrl` (ESPHome config implementing the full `uhf/pol-ctrl` four-plane — authoritative wire contract in `m5stamp-pol-ctrl/docs/m5stamp-pol-ctrl-mqtt-api.md`), and the Tier-2 console surface (`hf_console` X-Quad polarization panel, `set_pol` retained steady-state publishing, unit-tested). Both halves are built and unit-tested; neither is live. The controller is NOT deployed — PLC #2 runs pre-OTA field firmware, so the first flash must be physical USB (same constraint as PLC #1), a bench step not yet done. **Closure requires exactly:** (a) the physical USB flash of PLC #2 with `m5stamp-pol-ctrl`, and (b) a live `set_pol` → `/state` verification — the AE4 bench acceptance checklist in `m5stamp-pol-ctrl/docs/m5stamp-pol-ctrl-mqtt-api.md` §9 (boot honesty, interlock scope-verified, retained-cmd self-heal, loud invalid-value rejection, web auth) run on the real device. Do not close this entry on code review alone.
