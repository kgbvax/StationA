...
// injection) plus full receive/send logs let the tests assert exact packet
// sequences, loss recovery and bounded buffers.
...
// fakeRadioID is the 4-byte ID the fake radio presents as sentid (the real
// radio picks a random one; the value itself is irrelevant to the client).
...
func (fr *fakeRadio) handle(sock string, src *net.UDPAddr, data []byte) {
	...
	case h.len == ctrlLen:
		...
		case ptAreYouThere:
			...
			fr.send(sock, controlPacket(ptIAmHere, 0, fakeRadioID, h.sentID))
		case ptAreYouReady:
			if fr.noReady || fr.stopped {
				return
			}
			fr.send(sock, controlPacket(ptIAmReady, 1, fakeRadioID, h.sentID))
		case ptRetransmit:
	...
	case h.len == pingLen && h.typ == ptPing:
		if data[replyOff] == 0x00 && !fr.stopped {
			...
			p := make([]byte, pingLen)
			putHeader(p, header{len: pingLen, typ: ptPing, seq: h.seq, sentID: fakeRadioID, rcvdID: h.sentID})
			p[replyOff] = 0x01
...
func (fr *fakeRadio) answerRetransmit(sock string, seqs []uint16) {
	...
	for _, s := range seqs {
		...
		if d, ok := win[s]; ok {
			...
			// (the wfview behavior).
			fr.send(sock, controlPacket(ptIdle, s, fakeRadioID, 0))
		}
	}
}
...
func (fr *fakeRadio) replyLogin(sock string, h header, req []byte) {
	...
	putHeader(p, header{len: loginRespLen, sentID: fakeRadioID, rcvdID: h.sentID})
...
func (fr *fakeRadio) replyToken(sock string, h header, req []byte) {
	...
	putHeader(p, header{len: tokenLen, sentID: fakeRadioID, rcvdID: h.sentID})
...
func (fr *fakeRadio) replyStreamReq(sock string, h header, req []byte) {
	...
	putHeader(p, header{len: statusLen, sentID: fakeRadioID, rcvdID: h.sentID})
...
func (fr *fakeRadio) injectCIVLocked(payload []byte) {
	...
	putHeader(p, header{len: uint32(len(p)), sentID: fakeRadioID})
	p[civReplyOff] = 0xc1
...
func (fr *fakeRadio) injectCIVAt(seq uint16, payload []byte) {
	...
	fr.civSubSeq++
	p := make([]byte, civHeaderLen+len(payload))
	putHeader(p, header{len: uint32(len(p)), sentID: fakeRadioID, seq: seq})
	p[civReplyOff] = 0xc1
	binary.LittleEndian.PutUint16(p[civDatalenOff:], uint16(len(payload)))
	binary.BigEndian.PutUint16(p[civSendSeqOff:], fr.civSubSeq)
	copy(p[civHeaderLen:], payload)
...
func (fr *fakeRadio) injectPingRequest(seq uint16, uptimeMS uint32) {
	...
	defer fr.mu.Unlock()
	p := make([]byte, pingLen)
	putHeader(p, header{len: pingLen, typ: ptPing, seq: seq, sentID: fakeRadioID})
	p[replyOff] = 0x00
	...
	if fr.sawClientCiv {
		if d := seqDiff(h.seq, fr.clientSeq); d >= 1 && d <= maxMissingCap {
			for s := fr.clientSeq; s != h.seq; s++ {
				fr.send(sock, controlPacket(ptRetransmit, s, fakeRadioID, h.sentID))
			}
		}
	}
