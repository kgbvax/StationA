...
**CI-V layer:** address `A2` (radio), `E0` (controller); framing `FE FE A2 E0 <cmd> [<sub>] [<data>] FD`; OK reply `...FB FD`, NG `...FA FD` (rejection — e.g. freq/mode sets in satellite or memory mode). Frequency = 10 BCD digits little-endian, 10 Hz -> 100 MHz digit (144.500.000 -> `00 00 50 44 01`; U3 note: earlier copies of this brief carried `41` in the third byte — a typo that would encode 141.5 MHz. The hamlib `to_bcd` / wfview behavior — digits LSB-first, low nibble = lower digit — is authoritative). Mode = 2 bytes (mode + filter): `00` LSB, `01` USB, `02` AM, `03` CW, `04` RTTY, `05` FM, `07` CW-R (normalizes to `cw` on the bus — reverse-filter bit is not representable), `08` RTTY-R, `17` DV (unsupported on the bus), `22` DD (unsupported); data mode = `06 <00/01> <filter>` modifier.
