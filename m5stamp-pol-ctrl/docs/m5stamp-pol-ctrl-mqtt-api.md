...
## 8. Exposure register (KTD4, sixth vector) — recorded in `docs/known-issues.md`

The sixth vector of the sat-ops exposure review — the phase controller's own
device surfaces (web_server digest auth, OTA password, native-API `api_key`
encryption, MQTT `/cmd` via the shack broker and the HA bridge's inbound
forwarding, captive-portal fallback; FlexRadio relay fully `internal`) — is
decided and recorded in the register: `../docs/known-issues.md`, "Sat-ops
rotators: pre-deploy exposure review" → "Phase-controller device surfaces —
sixth vector". Status: decided 2026-09-13, **enforced at the PLC #2 flash** (the
flash is still future — first flash is physical USB; see §9).
