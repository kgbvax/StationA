...
echo "   NOTE: an empty AZ_PORT/EL_PORT selects the in-process mock device per axis,"
echo "   and mock telemetry publishes under the real slot addresses — pin the real"
echo "   /dev/serial/by-id identities at the bench bring-up (AZ_PORT/EL_PORT) before"
echo "   deploying against live antennas; config.example.toml ships mock ports deliberately."